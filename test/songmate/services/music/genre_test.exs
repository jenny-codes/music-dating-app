defmodule Songmate.MusicServiceGenreTest do
  ...
  describe "batch_create_genres/2" do
    test "returns a list of Genre records for every input item" do
      existing_attrs = valid_genre_attrs()
      MusicService.create_genre(existing_attrs)
      new_attrs = %{name: "to be inserted"}

      result = MusicService.batch_create_genres([existing_attrs, new_attrs], order: true)

      assert [%Genre{}, %Genre{}] = result
    end

    test "when input is empty, returns empty" do
      assert Enum.empty?(MusicService.batch_create_genres([], order: true))
    end
    ...
    test "when order: true return the records in the same order as input" do
      ...
      first_result = MusicService.batch_create_genres(first_input, order: true)
      assert_same_list_by(:name, first_input, first_result)

      second_input = [attrs2, attrs1, attrs3]
      second_result = MusicService.batch_create_genres(second_input, order: true)
      assert_same_list_by(:name, second_input, second_result)

      third_input = [attrs3, attrs2, attrs1]
      third_result = MusicService.batch_create_genres(third_input, order: true)
  ...
  describe "genres" do
    ...
    test "create_genre/1 with valid data creates a genre" do
      assert {:ok, %Genre{} = genre} = MusicService.create_genre(valid_genre_attrs())
      assert genre.name == "Modern Rock"
    end

    test "create_genre/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = MusicService.create_genre(@invalid_attrs)
    end

    test "update_genre/2 with valid data updates the genre" do
      genre = create_genre()
      assert {:ok, %Genre{} = genre} = MusicService.update_genre(genre, @update_attrs)
      assert genre.name == "some updated name"
    end

    test "update_genre/2 with invalid data returns error changeset" do
      genre = create_genre()
      assert {:error, %Ecto.Changeset{}} = MusicService.update_genre(genre, @invalid_attrs)
    end
  end
end

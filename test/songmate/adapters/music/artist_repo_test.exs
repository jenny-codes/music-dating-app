defmodule Songmate.ArtistRepoTest do
  ...
  describe "batch_get_or_create_artists/2" do
    test "returns a list of Artist records for every input item" do
      ...
      ArtistRepo.create_artist(existing_attrs)
      new_attrs = %{name: "to be inserted", spotify_id: "dont really care"}

      result = ArtistRepo.batch_get_or_create_artists([existing_attrs, new_attrs], order: true)

      assert [%Artist{}, %Artist{}] = result
    end

    test "when input is empty, returns empty" do
      assert Enum.empty?(ArtistRepo.batch_get_or_create_artists([], order: true))
    end
    ...
    test "when order: true return the records in the same order as input" do
      ...
      first_result = ArtistRepo.batch_get_or_create_artists(first_input, order: true)
      assert_same_list_by(:spotify_id, first_input, first_result)

      second_input = [attrs2, attrs1, attrs3]
      second_result = ArtistRepo.batch_get_or_create_artists(second_input, order: true)
      assert_same_list_by(:spotify_id, second_input, second_result)

      third_input = [attrs3, attrs2, attrs1]
      third_result = ArtistRepo.batch_get_or_create_artists(third_input, order: true)
  ...
  describe "artists" do
    ...
    test "create_artist/1 with valid data creates a artist" do
      assert {:ok, %Artist{} = artist} = ArtistRepo.create_artist(valid_artist_attrs())
    ...
    test "create_artist/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = ArtistRepo.create_artist(@invalid_attrs)
    end

    test "update_artist/2 with valid data updates the artist" do
      artist = artist_fixture()
      assert {:ok, %Artist{} = artist} = ArtistRepo.update_artist(artist, @update_attrs)
    ...
    test "update_artist/2 updates the associated genres" do
      ...
      {:ok, updated_artist} = ArtistRepo.update_artist(artist, %{genres: [%{name: "Jazz"}]})
    ...
    test "update_artist/2 with invalid data returns error changeset" do
      artist = artist_fixture()
      assert {:error, %Ecto.Changeset{}} = ArtistRepo.update_artist(artist, @invalid_attrs)
    end
  end
end

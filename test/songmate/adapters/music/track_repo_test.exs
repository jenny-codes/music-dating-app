defmodule Songmate.MusicTest do
  ...
  describe "batch_get_or_create_tracks/2" do
    test "returns a list of Track records for every input item" do
      ...
      TrackRepo.create_track(existing_attrs)
      new_attrs = %{name: "to be inserted", spotify_id: "dont really care"}

      result = TrackRepo.batch_get_or_create_tracks([existing_attrs, new_attrs], order: true)

      assert [%Track{}, %Track{}] = result
    end

    test "when input is empty, returns empty" do
      assert Enum.empty?(TrackRepo.batch_get_or_create_tracks([], order: true))
    end
    ...
    test "when order: true return the records in the same order as input" do
      ...
      first_result = TrackRepo.batch_get_or_create_tracks(first_input, order: true)
      assert_same_list_by(:spotify_id, first_input, first_result)

      second_input = [attrs2, attrs1, attrs3]
      second_result = TrackRepo.batch_get_or_create_tracks(second_input, order: true)
      assert_same_list_by(:spotify_id, second_input, second_result)

      third_input = [attrs3, attrs2, attrs1]
      third_result = TrackRepo.batch_get_or_create_tracks(third_input, order: true)
  ...
  describe "tracks" do
    ...
    test "create_track/1 with valid data creates a track" do
      assert {:ok, %Track{} = track} = TrackRepo.create_track(valid_track_attrs())
    ...
    test "create_track/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = TrackRepo.create_track(@invalid_attrs)
    end

    test "update_track/2 with valid data updates the track" do
      track = track_fixture()
      assert {:ok, %Track{} = track} = TrackRepo.update_track(track, @update_attrs)
      assert track.isrc == "USMRG0467010"
    ...
    test "update_track/2 with invalid data returns error changeset" do
      track = track_fixture()
      assert {:error, %Ecto.Changeset{}} = TrackRepo.update_track(track, @invalid_attrs)
    end
  end
end
